#include "AT_PlayMontageWithEvent.h"
#include "AbilitySystemComponent.h"
#include "AbilitySystemGlobals.h"
#include "ProjectH/Characters/BaseCharacter.h"

UAT_PlayMontageWithEvent::UAT_PlayMontageWithEvent()
{
	Rate = 1.f;
...
void UAT_PlayMontageWithEvent::Activate()
{
	...
	if (UAnimInstance* AnimInstance = ActorInfo->GetAnimInstance())
	{
			...
			, StartSection) > 0.f)
		{
			...
			CancelledHandle = Ability->OnGameplayAbilityCancelled.AddUObject(
				this, &UAT_PlayMontageWithEvent::OnAbilityCancelled);

			// Anim Montage의 Blend Out시에 대한 Delegate 등록
			BlendingOutDelegate.BindUObject(
				this, &UAT_PlayMontageWithEvent::OnMontageBlendingOut);
			...
			MontageEndedDelegate.BindUObject(
				this, &UAT_PlayMontageWithEvent::OnMontageEnded);
			AnimInstance->Montage_SetEndDelegate(
				MontageEndedDelegate, PlayAnimMontage);
		...
		else
		{
			UE_LOG(LogTemp, Warning
					, TEXT(
						"PlayMontageAndWaitForEvent call to PlayMontage failed!"
					));
		}
	}
	else
...
UAT_PlayMontageWithEvent* UAT_PlayMontageWithEvent::InitialEvent(
	UGameplayAbility* OwningAbility, FName TaskInstanceName
	, UAnimMontage* MontageToPlay, FGameplayTagContainer EventTags
	, FGameplayEventData EventData, float Rate, FName StartSection
	, bool bStopWhenAbilityEnds, float AnimRootMotionTranslationScale)
{
	UAbilitySystemGlobals::NonShipping_ApplyGlobalAbilityScaler_Rate(Rate);

	UAT_PlayMontageWithEvent* NewEvent = NewAbilityTask<
		UAT_PlayMontageWithEvent>(OwningAbility, TaskInstanceName);
...
void UAT_PlayMontageWithEvent::OnGameplayEvent(FGameplayTag EventTag
...
void UAT_PlayMontageWithEvent::OnAbilityCancelled()
{
...
void UAT_PlayMontageWithEvent::OnMontageBlendingOut(
...
void UAT_PlayMontageWithEvent::OnMontageEnded(UAnimMontage* Montage
											, bool bInterrupted)
{
	if (!bInterrupted)
	{
...
bool UAT_PlayMontageWithEvent::StopPlayingMontage()
{
	const FGameplayAbilityActorInfo* ActorInfo = Ability->GetCurrentActorInfo();
	if (!ActorInfo)
	{
...
FString UAT_PlayMontageWithEvent::GetDebugString() const
{
	UAnimMontage* PlayingMontage = nullptr;
	if (Ability)
	{
		const FGameplayAbilityActorInfo* ActorInfo = Ability->
			GetCurrentActorInfo();
		UAnimInstance* AnimInstance = ActorInfo->GetAnimInstance();
...
void UAT_PlayMontageWithEvent::ExternalCancel()
{
...
void UAT_PlayMontageWithEvent::OnDestroy(bool AbilityEnded)
{
	if (Ability)
	{
